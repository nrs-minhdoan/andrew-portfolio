...
import { ACCENT_GRADIENT_REVERSED, STAGGER } from "@/lib/theme";

/* Sort by award date descending — most recent first. */
const ITEMS = [...ACHIEVEMENTS].sort(
      ...
      <div className="container-page">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
          <ul className="grid gap-3 sm:grid-cols-2">
            {ITEMS.map((item, i) => (
              <Reveal as="li" key={item.slug} delay={i * STAGGER}>
                <div className="glass group relative h-full overflow-hidden rounded-2xl p-5 transition-all hover:-translate-y-0.5 hover:border-(--accent)/40">
                  <div className="mb-3 inline-flex items-center gap-2">
                    <IconBox size="sm" style={{ background: ACCENT_GRADIENT_REVERSED }}>
